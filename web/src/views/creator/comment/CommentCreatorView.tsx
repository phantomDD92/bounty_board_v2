...
import { deleteCommentForUser, getCommentListForUser } from '@/lib/api'
...
import CommentEditDrawer from './CommentEditDrawer'
...
type CommentWithActionsType = CommentType & {
  actions?: string
}
...
// Column Definitions
const columnHelper = createColumnHelper<CommentWithActionsType>()

const CommentCreatorView = () => {
  // States
  ...
  const handleDelete = () => {
    ...
    deleteCommentForUser(selected._id)
      .then(() => {
        toast.success(`Comment deleted successfully`);
        getCommentListForUser().then(newData => {
          setData(newData)
        }).catch(() => { })
      }).catch((error: any) => {
  ...
  const table = useReactTable({
    data: filteredData as CommentType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  return (
    <>
      <Card>
        <CardHeader title='Comment List' className='pbe-4' />
      ...
      {selected &&
        <>
          ...
          />
          <ConfirmDialog
            question='Are you sure to delete the comment?'
            data={selected}
            open={confirmShow}
...
export default CommentCreatorView
