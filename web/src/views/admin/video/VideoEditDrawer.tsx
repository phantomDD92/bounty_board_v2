...
import { changeVideoForAdmin } from '@/lib/api'

import type { VideoParamType, VideoType } from '@/types/valueTypes'
...
const VideoEditDrawer = ({ data, open, onClose, onUpdate }: Props) => {
  ...
  // Handle Form Submit
  const handleFormSubmit = async (params: VideoParamType) => {
    try {
      if (data) {
        await changeVideoForAdmin(data._id, params);
        onUpdate && onUpdate();
