...
import { deleteSocialForAdmin, getSocialListForAdmin, publishSocialForAdmin, weighSocialForAdmin } from '@/lib/api'

import type { SocialType, PublishType } from '@/types/valueTypes'
...
import SocialEditDrawer from './SocialEditDrawer'
...
type SocialWithActionsType = SocialType & {
  actions?: string
}
...
// Column Definitions
const columnHelper = createColumnHelper<SocialWithActionsType>()

const SocialAdminView = () => {
  // States
  ...
  const [data, setData] = useState<SocialType[]>([])
  const [filteredData, setFilteredData] = useState<SocialType[]>([])
  const [globalFilter, setGlobalFilter] = useState('')
  const [status, setStatus] = useState('0');

  useEffect(() => {
    getSocialListForAdmin()
      .then(newData => {
        setData(newData)
      })
      .catch(() => { })
  ...
  const handlePublish = async (params: PublishType) => {
    ...
    publishSocialForAdmin(selected._id, params)
      .then(() => {
        toast.success(`Social updated successfully`);
        getSocialListForAdmin().then(newData => {
          setData(newData)
        })
      })
      .catch((error: any) => {
  ...
  const handleDelete = async () => {
    ...
    deleteSocialForAdmin(selected._id)
      .then(() => {
  ...
  const handleUndo = () => {
    setUndoShow(false);
    publishSocialForAdmin(selected._id, { status: Status.PENDING, feedback: "" })
      .then(() => {
        toast.success(`Social updated successfully`);
        getSocialListForAdmin().then(newData => {
          setData(newData)
  ...
  const handleChangeWeight = (social: SocialType, value: number | null) => {
    weighSocialForAdmin(social._id, value ? value * 2 : 1)
      .then(() => {
        const newData: SocialType[] = [];

        Object.assign(newData, data)
        const current = newData.findIndex(el => el._id == social._id)
  ...
  const columns = useMemo<ColumnDef<SocialWithActionsType, any>[]>(
    () => [
      // {
      //   id: 'select',
      //   header: ({ table }) => (
  ...
  const table = useReactTable({
    data: filteredData as SocialType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  return (
    <>
      <Card>
        <CardHeader title='Social List' className='pbe-4' />
      ...
      {selected && (
        <>
          ...
          />
          <ConfirmDialog
            question='Are you sure to delete the social?'
            data={selected}
            open={confirmShow}
          ...
          />
          <ConfirmDialog
            question='Are you sure to unpublish the social?'
            data={selected}
            open={undoShow}
...
export default SocialAdminView
